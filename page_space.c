...
static void display_state(const char *state)
{
     ...
     * First line has:
     *
     * ST: + up to 17 characters
     *
     * Total: 4 + 17 = 21 characters.
     */

    len = strlen(state);

    if (len <= 17) {
        snprintf(line, sizeof(line), "ST: %s", state);
        menu_line(5, line);
        return;
    }
    ...
    /*
     * Prefer breaking at the last space so that words
    ...
    if (last_space) {
        size_t first_len = (size_t)(last_space - first);
...
void page_space(void)
{
    ...
    if (find_md_device(device, sizeof(device)) != 0) {
        ...
        menu_line(4, "LEVEL: N/A");
        menu_line(5, "ST: OFFLINE");
    ...
    if (rebuild >= 0) {
        ...
        menu_line(7, "HEALTH: OK");
    }
    else if (strstr(info.state, "degraded")) {
        menu_line(7, "HEALTH: DEGRADED");
    }
    else {
        menu_line(7, "HEALTH: UNKNOWN");
    }
}#include <stdio.h>
#include <stdlib.h>
#include <string.h>
